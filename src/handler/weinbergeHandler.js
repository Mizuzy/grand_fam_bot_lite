...
async function CreateFortyEmbed(guildName) {
    ...
    const baseEmbed = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# Weinberge  ${timeKey}:15`),
        )
...
module.exports = function startHotelwarHandler(client) {
    cron.schedule("05 20 * * *", async () => {
        ...
        const [rows] = await db.execute(
            "SELECT `setconfig` FROM config WHERE config = 'send_weinberge'"
        );
